  ...
  render() {
    const dataSeat = [ 
      ...
      },
      {
        ...
        "hang": "box",
        "danhSachGhe":[
          {"soGhe":"box1","gia":300000,"daDat": false},
          {"soGhe":"box2","gia":300000,"daDat": false},
          {"soGhe":"box3","gia":300000,"daDat": false},
          {"soGhe":"box4","gia":300000,"daDat": true},
          {"soGhe":"box5","gia":300000,"daDat": false},
          {"soGhe":"box6","gia":300000,"daDat": false}
        ]
      }
    
    ]
    const handleClickChoose =(seat)=>{
      if(!seat.daDat){
        const action={
          type:'set_status',
          payload:{
    ...
    return (
      <div className='seats'>
        ...
        <div className="rowSeat">
            {dataSeat[0]['danhSachGhe'].map((row,index)=>{
                return(<div key={index} className="rowNumber">{row.soGhe}</div>)
            })}
        </div>
        <div className="colSeat">
            { dataSeat.map((col,index)=>{
                return (
                  <div  key={index}  className="rowOfAlphabel">
                    <div className="colAphabel">{col.hang}</div>
                    <div className="seatByRowAlphabel">
                      {col.danhSachGhe.map((seat,index)=>{
                         return <div onClick={()=>{
                          handleClickChoose(seat)
                         }} key={`${index}seat`} id={seat.soGhe} className={`seat ${
                          //handle choosed seat
                          (seat.daDat == false ? (this.props.eachSeat.cartSeat.includes(seat.soGhe) ? 'choosed' : ''): 'disable')
                        }`}>{seat.soGhe}</div>
                      })}
                    </div>
                  </div>
        ...
        <div className="showWay">
          <p><i className="fa-solid fa-arrow-left"></i> lối ra</p>
          <div className="centerWay"><i className="fa-solid fa-arrow-up"></i><p>lối đi</p></div>
          <p>lối ra <i className="fa-solid fa-arrow-right"></i></p>
        </div>
      </div>
    )
  }
...
const mapStateToProps = (rootReducer)=>{
  return{
    eachSeat : rootReducer.chooseReducer,
  }
}
